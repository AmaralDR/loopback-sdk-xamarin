...
module.exports = function generateServices(app, ngModuleName, apiUrl) {
  ...
  return ejs.render(servicesTemplate, objOptions);
};
...
function describeModels(app) {
  ...
  app.handler('rest').adapter.getClasses().forEach(function (c) {
    ...
    if (app.models[c.name].definition.settings.xamarinIgnore) {
      return false;
    }
    ...
    // if (c.params) {
    //   for (item in Object.keys(c.params){
    //     if (item.type.name == 'ModelConstructor') {
    //       item.type.name = item.type.modelName;
    //     }
    //   };
    // }

    if (name.toLowerCase() == 'pessoa') {
      console.info((c.methods[c.methods.length - 6]).returns);
    }
    result[name] = c;
